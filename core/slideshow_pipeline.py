...
from core.prompt_loader import DOCS_PROMPTS_DIR, load_fenced_prompt
...
def parse_scene_script_response(content: str) -> list[dict[str, Any]]:
    """Parse script writer JSON and return exactly SCENE_COUNT scenes."""
    try:
    ...
    for index, scene in enumerate(scenes):
        ...
        for field in ("title", "description", "tts"):
...
def run_slideshow_pipeline(
    ...
    *,
    ...
    """Run scene script writer → slide images → per-scene TTS → pipeline_payload.json."""
