...
|----|------|-----------|-------|
| T045 | Retire the ending slide + fix slide/voice drift | 2026-07-17 | Ending slide + brand end card were two closing beats in a row. Script writer now returns intro + 3 content only (`TOTAL_SLIDE_COUNT = CONTENT_SCENE_COUNT + 1`); `drop_ending_slides()` strips the slide from drafts frozen earlier, so all 20 cached jobs kept their scripts/images (no regen spend). Removing it exposed ADR 0005's even split counting the narration-less intro as a slide: drift went from oscillating (±7.5s) to a flat **+10s lag** on a 60s video. `apply_narration_slide_timing` now takes content windows from `audio.scene_timestamps`; intro holds `INTRO_HOLD_MS` (2500, capped at half of scene 1). ADR 0009. |
